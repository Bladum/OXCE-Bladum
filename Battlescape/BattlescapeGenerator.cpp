 ...
 */
void BattlescapeGenerator::run( int ufoDamage )
{
	_save->setAlienCustom(_alienCustomDeploy ? _alienCustomDeploy->getType() : "", _alienCustomMission ? _alienCustomMission->getType() : "");
	...
	deployAliens(_alienCustomDeploy ? _alienCustomDeploy : ruleDeploy, ufoDamage);
 ...
 */
void BattlescapeGenerator::deployAliens(const AlienDeployment *deployment, int ufoDamage )
{
		...
		for (int i = 0; i < quantity; ++i)
		{
			...
			if (unit)
			{
				...
				else if (ufoDamage > RNG::generate(0, 99) )
				{
					...
					unit->setEnergy(RNG::generate(5, unit->getEnergy() ) );
					unit->moraleChange( -RNG::generate(0, 99) );
					unit->setHealth(RNG::generate(5, unit->getHealth() ) );
				}
			}
		}
	}
